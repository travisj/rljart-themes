...
<head profile="http://gmpg.org/xfn/11">
	...
	<style type="text/css" media="screen">
		...
		/* BEGIN IMAGE CSS */
			...
			body	 	{ background: url("<?php bloginfo('stylesheet_directory'); ?>/images/springbgcolor.gif"); }	<?php /* Checks to see whether it needs a sidebar or not */ if ((! $withcomments) && (! is_single()) && (! is_page())) { ?>
			#page		{ background: url("<?php bloginfo('stylesheet_directory'); ?>/images/melon2bg.jpg") repeat-y top; border: none; } <?php } else { // No sidebar ?>
			#page		{ background: url("<?php bloginfo('stylesheet_directory'); ?>/images/springbgwide.gif") repeat-y top; border: none; } <?php } ?>
			#headerimg 	{ background: url("<?php bloginfo('stylesheet_directory'); ?>/images/springheader.gif") no-repeat bottom center; }
			...
			#header 	{ margin: 0 !important; margin: 0; padding: 1px; height: 150px; width: 960px; }
			#headerimg 	{ margin: 0; height: 150px; width: 960px; } 
		/* END IMAGE CSS */
